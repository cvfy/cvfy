...
return newState.auth.user.id
  ...
  useEffect(() => {
    ...
    axios
      .get(
      `http://localhost:5000/api/users/resume/cv/allCV/${getUserId()}`)
...
window.location.href = "http://localhost:3000/create-cv"
  ...
  return (
    <div className="CV_Dashboard_MainContainer">
      ...
      {resume.map(el => (<ScrollAnimation className="cvBox2" animateIn="fadeInUp delay-1s"><img onClick={() => setLocalStorage(el.id)} src={`http://localhost:5000/static/${el.id}.jpg`} alt={el.id}/></ScrollAnimation>) )}
    </div>
  );
};
